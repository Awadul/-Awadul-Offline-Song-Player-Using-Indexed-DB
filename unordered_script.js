...
addPlaylistButton.addEventListener("click", (event)=> {
    ...
    let newObjectSource;
    if (!event.hasOwnProperty("playlist_name")){
...
function openPlaylistConnection() {
    return new Promise((resolve, reject)=> {
        ...
        playlistDB_request.onsuccess = (event)=> {
            let playlist_db = event.target.result
            console.log(playlist_db)
            resolve(playlist_db)
        }
        playlistDB_request.onerror = (e)=> {
            console.log("error in opening playlist db")
...
function swapObectStores(playlistName, songDB_ver, playlistSongs){
    return new Promise ((resolve, reject)=> {
            ...
            openPlaylistConnection().then((playlist_db)=> {
                ...
                cursorRequest.onsuccess = (e)=> {
                    ...
                    if (cursor){
                        ...
                        // console.log("cursor problem value " , cursor.value)       // cursor returns the value of the object that was stored in the database */
                        // totalSongs++;
                        cursor.continue();
                    } else {
                        swapObectStores(playlistName, songDB_ver, tempSongs).then(()=> {
                            playlist_db.close();
                            first_load_songs_display();
                            console.log(songs)
                ...
                // console.log("songs from playlist clicked", songs)
            });
        });
        // openPlaylistConnection().then((playlist_db)=> {
        //     let tx = playlist_db.transaction([playlistName], "readonly")
